...
fn detect_install_method() -> InstallMethod {
    let exec_path = std::env::current_exe()
        ...
        .unwrap_or_default();
    if exec_path.contains(".kfcode/bin") || exec_path.contains(".local/bin") {
...
async fn latest_version(method: InstallMethod) -> anyhow::Result<String> {
    ...
    match method {
        InstallMethod::Brew => {
            let response = client
                // TODO(KFCode): KFCode 暂无 Homebrew formula；URL 保留以避免运行时 404。
                .get("https://formulae.brew.sh/api/formula/kfcode.json")
                .send()
                .await?;
        ...
        InstallMethod::Npm | InstallMethod::Pnpm | InstallMethod::Bun => {
            let response = client
                .get("https://registry.npmjs.org/kfcode-ai/latest")
                .send()
                .await?;
            let json: serde_json::Value = parse_http_json(response).await?;
            let version = json
        ...
        InstallMethod::Scoop => {
            let response = client
                // TODO(KFCode): KFCode 暂无 Scoop manifest；URL 保留以避免运行时 404。
                .get("https://raw.githubusercontent.com/ScoopInstaller/Main/master/bucket/kfcode.json")
                .send()
                .await?;
            let json: serde_json::Value = parse_http_json(response).await?;
            let version = json
        ...
        _ => {
            let response = client
                ...
                .header("User-Agent", "kfcode-cli-rust-rewrite")
                .send()
                .await?;
            let json: serde_json::Value = parse_http_json(response).await?;
            let tag = json
...
fn run_upgrade_process(method: InstallMethod, target: &str) -> anyhow::Result<()> {
    let status = match method {
        InstallMethod::Curl => ProcessCommand::new("sh")
            .arg("-c")
            .arg("curl -fsSL https://kfcode.ai/install | bash")
            .env("VERSION", target)
            .status(),
        InstallMethod::Npm => ProcessCommand::new("npm")
            .args(["install", "-g", &format!("kfcode-ai@{}", target)])
            .status(),
        ...
        InstallMethod::Choco => ProcessCommand::new("choco")
            .args(["upgrade", "kfcode", "--version", target, "-y"])
            .status(),
        InstallMethod::Scoop => ProcessCommand::new("scoop")
            .args(["install", &format!("kfcode@{}", target)])
            .status(),
        InstallMethod::Unknown => {
...
async fn handle_upgrade_command(
    ...
    if current == target {
        println!("kfcode upgrade skipped: {} is already installed", target);
        return Ok(());
    }
